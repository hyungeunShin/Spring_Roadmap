...
public class AopApplication {
	...
	핵심 기능과 부가 기능을 분리
		...
		스프링이 제공하는 어드바이저도 어드바이스(부가 기능)와 포인트컷(적용 대상)을 가지고 있어서 개념상 하나의 애스펙트이다.
	...
	AspectJ 프레임워크
		...
		물론 스프링도 AOP 를 지원하지만 대부분 AspectJ의 문법을 차용하고, AspectJ가 제공하는 기능의 일부만 제공한다.
	...
	AOP 적용 방식
		...
		AOP 를 사용할 때 부가 기능 적용 방식은 크게 3가지 방법이 있다.
			- 컴파일 시점
			...
			- 클래스 로딩 시점
				...
				수 많은 모니터링 툴들이 이 방식을 사용한다. 이 시점에 Aspect 를 적용하는 것을 로드 타임 위빙이라 한다.
			...
			부가 기능이 적용되는 차이를 정리하면 다음과 같다.
				...
				따라서 항상 프록시를 통해야 부가 기능을 사용할 수 있다. 스프링 AOP 는 이 방식을 사용한다.
	...
	AOP 적용 위치
		...
		참고
		스프링은 AspectJ의 문법을 차용하고 프록시 방식의 AOP 를 적용한다. AspectJ를 직접 사용하는 것이 아니다.

		스프링이 제공하는 AOP 는 프록시를 사용한다.
		따라서 프록시를 통해 메서드를 실행하는 시점에만 AOP 가 적용된다.
		AspectJ를 사용하면 앞서 설명한 것 처럼 더 복잡하고 더 다양한 기능을 사용할 수 있다.
		그렇다면 스프링 AOP 보다는 더 기능이 많은 AspectJ를 직접 사용해서 AOP 를 적용하는 것이 더 좋지 않을까?
		AspectJ를 사용하려면 공부할 내용도 많고, 자바 관련 설정(특별한 컴파일러, AspectJ 전용 문법, 자바 실행 옵션)도 복잡하다.
		반면에 스프링 AOP 는 별도의 추가 자바 설정 없이 스프링만 있으면 편리하게 AOP 를 사용할 수 있다.
		스프링이 제공하는 AOP 기능만 사용해도 대부문의 문제를 해결할 수 있다.
